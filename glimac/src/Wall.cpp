...
namespace glimac {
	...
	void Wall::draw(int i, int j, glm::mat4 &viewMatrix, SDLWindowManager &window) const {
		...
		glUniform3f(uKd, 0.4f, 0.3f, 0.05f); //couleur diffuse
		glUniform3f(uKs, 0.4f, 0.3f, 0.05f); //couleur tache speculaire
		...
		glBindVertexArray(model.VAO());
			glm::mat4 MVMatrix = viewMatrix*glm::translate(glm::mat4(1.0), glm::vec3(j, 1.2, -i));
